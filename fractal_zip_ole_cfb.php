...
/**
 * Pack a mostly-zero OLE template into non-zero runs (cuts CLASSIC peel tax).
 * Wire: "FZOT" + u32 total_len + u32 nruns + (u32 off, u32 len, bytes)*n
 *
 * @return string packed blob, or original template when packing does not shrink
 */
function fractal_zip_ole_pack_sparse_template(string $template): string {
	$n = strlen($template);
	if ($n < 64) {
		return $template;
	}
	...
	while ($i < $n) {
		...
		$runs[] = array($start, $i - $start);
	}
	if ($runs === array()) {
		// All zeros — still smaller as empty-run sparse.
		$packed = 'FZOT' . pack('VV', $n, 0);
		return (strlen($packed) < $n) ? $packed : $template;
	}
	$packed = 'FZOT' . pack('VV', $n, count($runs));
	foreach ($runs as $r) {
		$off = (int) $r[0];
		$len = (int) $r[1];
		$packed .= pack('VV', $off, $len) . substr($template, $off, $len);
	}
	return (strlen($packed) < $n) ? $packed : $template;
}

/** Expand FZOT sparse template, or return input unchanged when not sparse. */
function fractal_zip_ole_unpack_sparse_template(string $packed): ?string {
	if (!str_starts_with($packed, 'FZOT') || strlen($packed) < 12) {
		return $packed;
	}
	$hdr = unpack('Vlen/Vnr', substr($packed, 4, 8));
	if ($hdr === false) {
		return null;
	}
	$total = (int) $hdr['len'];
	$nr = (int) $hdr['nr'];
	if ($total < 0 || $total > 64 * 1024 * 1024 || $nr < 0 || $nr > 100000) {
		return null;
	}
	$buf = str_repeat("\0", $total);
	$off = 12;
	for ($i = 0; $i < $nr; $i++) {
		if ($off + 8 > strlen($packed)) {
			return null;
		}
		$rh = unpack('Voff/Vlen', substr($packed, $off, 8));
		if ($rh === false) {
			return null;
		}
		$ro = (int) $rh['off'];
		$rl = (int) $rh['len'];
		$off += 8;
		if ($ro < 0 || $rl < 0 || $ro + $rl > $total || $off + $rl > strlen($packed)) {
			return null;
		}
		if ($rl > 0) {
			$chunk = substr($packed, $off, $rl);
			for ($j = 0; $j < $rl; $j++) {
				$buf[$ro + $j] = $chunk[$j];
			}
		}
...
function fractal_zip_ole_rebuild_from_template(string $template, array $streams): ?string {
	$unpacked = fractal_zip_ole_unpack_sparse_template($template);
	if ($unpacked === null) {
		return null;
	}
	$buf = $unpacked;
