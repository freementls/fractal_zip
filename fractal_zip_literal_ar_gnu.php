...
/**
 * List data members of a GNU/System V `ar` archive (skips // and /N tables).
 *
 * @return list<array{name: string, data: string}>|null
 */
function fractal_zip_literal_ar_list_file_members(string $bytes): ?array {
	$n = strlen($bytes);
	if ($n < 8 + 60 || substr($bytes, 0, 8) !== "!<arch>\n") {
		return null;
	}
	$out = array();
	$off = 8;
	$cap = 256;
	$eCap = getenv('FRACTAL_ZIP_FOLDER_AR_MAX_MEMBERS');
	if ($eCap !== false && (int) $eCap > 0) {
		$cap = max(1, (int) $eCap);
	}
	while ($off + 60 <= $n && count($out) < $cap) {
		$hdr = substr($bytes, $off, 60);
		if (!fractal_zip_literal_ar_gnu_header_fmag_ok($hdr)) {
			break;
		}
		$name16 = substr($hdr, 0, 16);
		$sz = fractal_zip_literal_ar_gnu_header_size($hdr);
		if ($sz < 0) {
			break;
		}
		...
		$pad = ($sz % 2) ? 1 : 0;
		$name = rtrim($name16, ' ');
		// Skip symbol/string tables and long-name indirections; keep short data names.
		if (fractal_zip_literal_ar_gnu_member_name_ok_for_data($name16)) {
			// GNU trailing slash on member name.
