 ...
 */
function fractal_zip_raster_canonical_ext_from_bytes(string $bytes): ?string {
...
function fractal_zip_raster_canonical_try(string $relPath, string $bytes): ?string {
	...
	if (!fractal_zip_content_format_policy_is_raster_member($relPath, $bytes)) {
		return null;
	}
	$ext = strtolower((string) pathinfo(str_replace('\\', '/', $relPath), PATHINFO_EXTENSION));
	if ($ext === 'jpeg') {
	...
	if ($ext === '' || !in_array($ext, fractal_zip_raster_canonical_extensions(), true)) {
		$ext = fractal_zip_raster_canonical_ext_from_bytes($bytes) ?? $ext;
	}
	if ($ext === '' || !in_array($ext, fractal_zip_raster_canonical_extensions(), true)) {
		return null;
	}
