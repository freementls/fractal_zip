...
function fractal_zip_literal_pac_preprocess_literal_for_bundle(string $relPath, string $rawBytes): string {
	...
	$rawBytes = fractal_zip_tokenize_fast_tier_preprocess_literal($relPath, $rawBytes);
...
/**
 * Repeatedly apply stream handlers (magic sniff first, then identify tier-1 extension hint) while size improves.
 */
function fractal_zip_literal_pac_preprocess_streams_multipass(string $relPath, string $rawBytes): string {
	...
	$idRow = fractal_zip_identify_for_policy($relPath, $rawBytes);
	$ext = strtolower((string) ($idRow['extension'] ?? ''));
	for ($i = 0; $i < $maxR; $i++) {
		$got = null;
		if (fractal_zip_content_format_policy_is_pdf_container($relPath, $rawBytes)) {
			$got = fractal_zip_literal_pac_try_stream_smaller('pdf', $rawBytes);
		}
		if ($got === null) {
...
/**
 * Name-only entropy score for soft-capping large ZIPs (higher = keep earlier).
 */
function fractal_zip_literal_pac_zip_member_name_score(string $name): int {
	$low = strtolower(str_replace('\\', '/', $name));
	$s = 0;
	if (preg_match('/\.(xml|rels|xhtml|html|htm|svg|css|js|json|yaml|yml|txt|csv|tsv|md|pa\.yaml)$/', $low)) {
		$s += 40;
	}
	if (preg_match('/\.(png|jpe?g|gif|webp|bmp|ico|woff2?|ttf|otf|dex|so|dll|class|pyc|mp[34]|m4a|ogg|opus)$/', $low)) {
		$s -= 25;
	}
	if (str_contains($low, '/media/') || str_contains($low, '/embeddings/')) {
		$s -= 30;
	}
	if ($low === 'datamodel' || str_starts_with($low, 'datamodel/')) {
		$s -= 50;
	}
	return $s;
}
 ...
 * when under cap; soft-selects by name score when member count exceeds the limit (never hard-aborts).
 *
 * @return list<array{name: string, data: string}>|null null if not a readable multi-file zip
 */
function fractal_zip_literal_pac_list_zip_members_for_mode18(string $compressed, ?int $maxTotalOverride = null): ?array {
	...
	if ($maxTotalOverride !== null) {
		$maxTotal = max(0, $maxTotalOverride);
	}
	[$inPath] = fractal_zip_literal_pac_temp_pair('zip');
	if (file_put_contents($inPath, $compressed) === false) {
		return null;
	...
	$cnt = $z->count();
	// n≥1: single-file ROM zips (82) still need member listing for semantic verify.
	if ($cnt < 1) {
		$z->close();
		@unlink($inPath);
		return null;
	...
	/** @var list<array{i: int, name: string, size: int, score: int}> $candidates */
	$candidates = [];
	for ($i = 0; $i < $cnt; $i++) {
		$stat = $z->statIndex($i);
		if ($stat === false) {
		...
		$candidates[] = [
			...
			'score' => fractal_zip_literal_pac_zip_member_name_score($name),
			'mtime' => (int) ($stat['mtime'] ?? 0),
			'comp_method' => (int) ($stat['comp_method'] ?? ZipArchive::CM_DEFLATE),
		];
	}
	...
	if (count($candidates) > $maxMembers) {
		usort($candidates, static function (array $a, array $b): int {
			$d = $b['score'] <=> $a['score'];
			if ($d !== 0) {
				return $d;
			}
			// Prefer smaller payloads when scores tie (more compressible / less entropy waste).
			$ds = $a['size'] <=> $b['size'];
			return $ds !== 0 ? $ds : ($a['i'] <=> $b['i']);
		});
		$candidates = array_slice($candidates, 0, $maxMembers);
		usort($candidates, static fn(array $a, array $b): int => $a['i'] <=> $b['i']);
	}

	$out = [];
	$total = 0;
	foreach ($candidates as $c) {
		$payload = $z->getFromIndex($c['i']);
		...
		$plen = strlen($payload);
		if ($maxTotal > 0 && ($total + $plen) > $maxTotal) {
			// Soft: skip oversized remainder instead of aborting the whole peel.
			continue;
		}
		$total += $plen;
		$row = ['name' => $c['name'], 'data' => $payload];
		if (isset($c['mtime'])) {
			$row['mtime'] = (int) $c['mtime'];
		}
		if (isset($c['comp_method'])) {
			$row['comp_method'] = (int) $c['comp_method'];
		}
		$out[] = $row;
	}
	$z->close();
...
/**
 * Set "version needed to extract" (uint16 LE) on every local + central header.
 * PHP/libzip often writes 10 for STORE entries; many real ZIPs use 20 everywhere.
 */
function fractal_zip_literal_pac_zip_set_version_needed(string $zip, int $versionNeeded): string {
	$versionNeeded = max(10, min(63, $versionNeeded));
	$ver = pack('v', $versionNeeded);
	$n = strlen($zip);
	$out = $zip;
	// Local file headers (sequential from offset 0 until central directory).
	$off = 0;
	while ($off + 30 <= $n && substr($out, $off, 4) === "PK\x03\x04") {
		$out = substr($out, 0, $off + 4) . $ver . substr($out, $off + 6);
		$nameLen = unpack('v', substr($out, $off + 26, 2))[1];
		$extraLen = unpack('v', substr($out, $off + 28, 2))[1];
		$compSize = unpack('V', substr($out, $off + 18, 4))[1];
		if ($nameLen < 0 || $extraLen < 0 || $compSize < 0) {
			break;
		}
		$next = $off + 30 + $nameLen + $extraLen + $compSize;
		if ($next <= $off || $next > $n) {
			break;
		}
		$off = $next;
	...
	$off = 0;
	while (($off = strpos($out, "PK\x01\x02", $off)) !== false) {
		if ($off + 46 > $n) {
			break;
		}
		// version needed at central+6 (after signature + version made by).
		$out = substr($out, 0, $off + 6) . $ver . substr($out, $off + 8);
		$nameLen = unpack('v', substr($out, $off + 28, 2))[1];
		$extraLen = unpack('v', substr($out, $off + 30, 2))[1];
		$commentLen = unpack('v', substr($out, $off + 32, 2))[1];
		$next = $off + 46 + $nameLen + $extraLen + $commentLen;
		if ($next <= $off) {
			break;
		}
		$off = $next;
...
/**
 * Rebuild a ZIP from decoded member payloads.
 * Honors optional per-member `comp_method` (0=store, 8=deflate) and `mtime`.
 *
 * @param list<array{name: string, data: string, mtime?: int, comp_method?: int}> $members
 * @param int $deflateLevel used for CM_DEFLATE entries (1–9)
 */
function fractal_zip_literal_pac_rebuild_zip_multi_semantic(array $members, int $deflateLevel = 9): ?string {
	// n≥1: single-file ROM zips (test_files82) need the same rebuild path as
	// multi-member silesia peels; semantic verify compares inner payloads.
	if (!class_exists(ZipArchive::class) || count($members) < 1) {
		return null;
	}
	$deflateLevel = max(1, min(9, $deflateLevel));
	...
	foreach ($members as $m) {
		...
		if (method_exists($z2, 'setCompressionIndex')) {
			$method = (int) ($m['comp_method'] ?? ZipArchive::CM_DEFLATE);
			if ($method === ZipArchive::CM_STORE || $method === 0) {
				$z2->setCompressionIndex($idx, ZipArchive::CM_STORE);
			} else {
				$z2->setCompressionIndex($idx, ZipArchive::CM_DEFLATE, $deflateLevel);
			}
		}
		if (isset($m['mtime']) && method_exists($z2, 'setMtimeIndex')) {
			$z2->setMtimeIndex($idx, (int) $m['mtime']);
		}
		$idx++;
	}
	if (!$z2->close()) {
	...
	// libzip writes version-needed=10 for STORE; most tool-made ZIPs use 20 for all entries.
	// Prefer 20 when it matches the original (matching loop); default here keeps deflate-only
	// archives stable and STORE hybrids closer to real-world containers.
	return fractal_zip_literal_pac_zip_set_version_needed($newC, 20);
}

/**
 * Find a bit-exact ZIP rebuild (deflate level sweep + member methods/mtimes).
 *
 * @param list<array{name: string, data: string, mtime?: int, comp_method?: int}> $members
 * @return array{bytes: string, level: int}|null
 */
function fractal_zip_literal_pac_rebuild_zip_multi_matching(array $members, string $original): ?array {
	if ($original === '') {
		return null;
	}
	// Prefer level 9 (extract default), then common fixture levels.
	foreach ([9, 8, 6, 1, 5, 7, 4, 3, 2] as $level) {
		$built = fractal_zip_literal_pac_rebuild_zip_multi_semantic($members, $level);
		if ($built !== null && $built === $original) {
			return ['bytes' => $built, 'level' => $level];
		}
	}
	return null;
}
...
/**
 * Encode ZIP_MULTI rebuild profile (methods/mtimes/level) as FP\x03[+inner patch].
 *
 * Wire: FP\x03 + u8 level + u8 flags + [methods…] + [mtimes…] + innerPatch
 * flags: bit0=per-member mtimes, bit1=per-member methods, bit2=shared mtime (one u32),
 *        bit3=STORE bitmap (ceil(n/8) bytes; bit i set => method 0 else 8; mutually exclusive with bit1),
 *        bit4=folder-default mtime (FZHR v2; expanded to bit2 on decode).
 *
 * @param list<array{name: string, data: string, mtime?: int, comp_method?: int}> $members
 */
function fractal_zip_folder_zip_rebuild_profile_patch(array $members, int $level, string $innerPatch = ''): string {
	if (!function_exists('fractal_zip_ensure_folder_per_member_best_loaded')) {
		// profile helpers live next to folder bundle; allow call from literal_pac
	}
	...
	$flags = 0;
	$sharedMtime = null;
	if ($allMtimesSet && $mtimes !== array()) {
		$uniq = array_values(array_unique($mtimes, SORT_NUMERIC));
		if (count($uniq) === 1) {
			// One mtime for every member — store once (flag bit 2).
			$flags |= 4;
			$sharedMtime = (int) $uniq[0];
		} else {
			$flags |= 1;
		}
	} elseif ($mtimes !== array()) {
		// Partial mtimes: fall back to per-member (zeros for missing).
		$flags |= 1;
	}
	$persistMethods = $hasStore || $level !== 9;
	$useStoreBitmap = false;
	$storeBitmap = '';
	if ($persistMethods) {
		$methods = array();
		$only08 = true;
		...
		// Compact form when every method is STORE(0) or DEFLATE(8): bit i set => STORE.
		if ($only08 && $hasStore) {
			$useStoreBitmap = true;
			$nMem = count($methods);
			$nb = (int) ceil($nMem / 8);
			$storeBitmap = str_repeat("\0", max(1, $nb));
			for ($i = 0; $i < $nMem; $i++) {
				if ($methods[$i] === 0) {
					$bi = $i >> 3;
					$storeBitmap[$bi] = chr(ord($storeBitmap[$bi]) | (1 << ($i & 7)));
				}
			}
		}
	}
	if ($persistMethods) {
		$flags |= $useStoreBitmap ? 8 : 2;
	}
	$parts = array('FP' . chr(3), chr($level & 0xFF), chr($flags & 0xFF));
	if ($persistMethods) {
		if ($useStoreBitmap) {
			$parts[] = $storeBitmap;
		} else {
			foreach ($members as $m) {
				$parts[] = chr(((int) ($m['comp_method'] ?? 8)) & 0xFF);
			}
		}
	}
	if (($flags & 4) !== 0) {
		$parts[] = pack('N', (int) $sharedMtime);
	} elseif (($flags & 1) !== 0) {
		foreach ($members as $m) {
			$parts[] = pack('N', (int) ($m['mtime'] ?? 0));
		}
	}
	return implode('', $parts) . $innerPatch;
}

/**
 * Apply FP\x03 rebuild profile onto ZIP member rows; returns [enrichedMembers, innerPatch, deflateLevel].
 *
 * @param list<array{name: string, data: string}> $zipMembers
 * @return array{0: list<array{name: string, data: string, mtime?: int, comp_method?: int}>, 1: string, 2: int}
 */
function fractal_zip_folder_zip_parse_rebuild_profile(array $zipMembers, string $patch): array {
	if (strlen($patch) < 5 || substr($patch, 0, 2) !== 'FP' || ord($patch[2]) !== 3) {
		return array($zipMembers, $patch, 9);
	}
	$level = max(1, min(9, ord($patch[3])));
	$flags = ord($patch[4]);
	$off = 5;
	$n = strlen($patch);
	$nMem = count($zipMembers);
	$methods = null;
	$mtimes = null;
	if (($flags & 8) !== 0) {
		// STORE bitmap: bit i set => STORE(0), clear => DEFLATE(8).
		$nb = (int) ceil($nMem / 8);
		if ($nb < 1) {
			$nb = 1;
		}
		if ($off + $nb > $n) {
			fractal_zip::fatal_error('Corrupt folder ZIP rebuild profile (store bitmap).');
		}
		$bitmap = substr($patch, $off, $nb);
		$off += $nb;
		$methods = array();
		for ($i = 0; $i < $nMem; $i++) {
			$bit = (ord($bitmap[$i >> 3]) >> ($i & 7)) & 1;
			$methods[] = $bit ? 0 : 8;
		}
	} elseif (($flags & 2) !== 0) {
		if ($off + $nMem > $n) {
			fractal_zip::fatal_error('Corrupt folder ZIP rebuild profile (methods).');
		}
		$methods = array();
		for ($i = 0; $i < $nMem; $i++) {
			$methods[] = ord($patch[$off + $i]);
		}
		$off += $nMem;
	}
	if (($flags & 4) !== 0) {
		// Shared mtime: one u32 applied to every member.
		if ($off + 4 > $n) {
			fractal_zip::fatal_error('Corrupt folder ZIP rebuild profile (shared mtime).');
		}
		$shared = unpack('N', substr($patch, $off, 4))[1];
		$off += 4;
		$mtimes = array_fill(0, $nMem, (int) $shared);
	} elseif (($flags & 1) !== 0) {
		if ($off + (4 * $nMem) > $n) {
			fractal_zip::fatal_error('Corrupt folder ZIP rebuild profile (mtimes).');
		}
		$mtimes = array();
		for ($i = 0; $i < $nMem; $i++) {
			$mtimes[] = unpack('N', substr($patch, $off, 4))[1];
			$off += 4;
		}
	}
	$enriched = array();
	foreach ($zipMembers as $i => $m) {
		$row = array(
			'name' => (string) $m['name'],
			'data' => (string) $m['data'],
		);
		if ($methods !== null) {
			$row['comp_method'] = (int) $methods[$i];
		} elseif (isset($m['comp_method'])) {
			$row['comp_method'] = (int) $m['comp_method'];
		}
		if ($mtimes !== null) {
			$row['mtime'] = (int) $mtimes[$i];
		} elseif (isset($m['mtime'])) {
			$row['mtime'] = (int) $m['mtime'];
		}
		$enriched[] = $row;
	}
	return array($enriched, substr($patch, $off), $level);
}
