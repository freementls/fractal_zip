...
/**
 * Harmony tier B: siteinfo-only phrase pack + pp96 stack (no full-blob tokens, no PAQ).
 *
 * Usage: php benchmarks/run_enwik8_harmony_siteinfo_encode.php [--name=harmony_siteinfo]
 */
...
fwrite(STDERR, "[enwik8_harmony_siteinfo] encode start\n");
$t0 = microtime(true);
$fz = new fractal_zip();
...
file_put_contents($outJson, json_encode(array(
	'generated' => date('c'),
	'bench_profile' => 'world-record-harmony-siteinfo',
	'cases' => array(array(
		'label' => 'test_files109',
...
fwrite(STDERR, "[enwik8_harmony_siteinfo] fzc={$fzcBytes} B " . number_format($sec, 1) . "s → {$outJson}\n");
