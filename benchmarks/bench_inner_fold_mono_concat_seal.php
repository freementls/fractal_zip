...
$tmp = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'fz_seal_mc_' . getmypid();
@mkdir($tmp, 0700, true);
...
foreach (array(
	'fast' => array('FRACTAL_ZIP_INNER_FOLD_FRACTAL_FAST' => '1'),
	'full' => array('FRACTAL_ZIP_INNER_FOLD_FRACTAL_FAST' => '0'),
	'gzip_only' => array('FRACTAL_ZIP_INNER_FOLD_FRACTAL' => '0'),
) as $label => $env) {
	foreach ($env as $k => $v) {
		putenv($k . '=' . $v);
	}
	if (!isset($env['FRACTAL_ZIP_INNER_FOLD_FRACTAL'])) {
	...
	$raw = (int) ($stats['fold_stat_bytes'] ?? 0);
	$gzOnly = -1;
	if ($raw > 0) {
		$inner = (string) ($stats['stat_pred_inner_bytes'] ?? '');
		if ($inner === '' && isset($GLOBALS['fractal_zip_enwik_last_stat_pred_inner_blob'])) {
			$inner = (string) $GLOBALS['fractal_zip_enwik_last_stat_pred_inner_blob'];
		}
	}
	echo $label
		. ' packed=' . $packed
