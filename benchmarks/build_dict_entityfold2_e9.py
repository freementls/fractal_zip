#!/usr/bin/env python3
"""Build Profile B2 dictionary: post-fold counts with FOLD2 (amp+nbsp/ndash/mdash).

Matches FXCM_ENTITY_FOLD + FXCM_ENTITY_FOLD2. Longer &amp;nbsp;/ndash/mdash forms
are removed before tokenization, so nbsp/ndash/mdash become near-dead like quot —
block them from new append slots (stock entries may remain).
"""
...
CACHE = f"{ROOT}/benchmarks/.ladder_cache/enwik9_postfold2_word_counts.tsv"
OUT_APPEND = f"{OUT_DIR}/english_entityfold2_e9.dic"
CAP = 44880
# Longest-first order matches preprocessor.cpp
FOLD = [
    (b"&amp;ndash;", bytes([7])),
    (b"&amp;mdash;", bytes([8])),
    (b"&amp;nbsp;", bytes([6])),
...
ENTITY_ONLY_BLOCKLIST = {b"quot", b"nbsp", b"ndash", b"mdash"}
...
def main() -> None:
    ...
    for w in (b"quot", b"amp", b"lt", b"gt", b"nbsp", b"ndash", b"mdash"):
        print(f"  postfold2 count[{w.decode()}]={counts.get(w, 0):,}", file=sys.stderr)
    ...
    print("top15 postfold2 append:", [(w.decode(), c) for _, c, w in cands[:15]], file=sys.stderr)
    for bad in ENTITY_ONLY_BLOCKLIST:
        if bad in chosen:
            raise SystemExit(f"BUG: {bad.decode()} leaked into append")
