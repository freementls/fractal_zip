#!/usr/bin/env bash
# Resume consistent ladder: A1 10m + B0 1m/10m (A1 1m already KEEP -15).
set -euo pipefail
...
screen() {
  ...
  wait_guard "$tag" || { echo "BLOCKED"; return 0; }
...
log "=== consistent ladder RESUME (after A1_1m KEEP -15) ==="

G=$(screen A1_dict365e9_10m "$RUN/cmix_lto" \
  "$ROOT/benchmarks/.ladder_cache/dicts/english_append365_e9.dic" "$SL10" "$B10_A0")
log "A1 10m gate=$G"
...
G=$(screen B0_entityfold_10m "$RUN/cmix_entity" \
  "$ROOT/benchmarks/.ladder_cache/dicts/english_entityfold_e9.dic" "$SL10" "$B10_A0")
log "B0 10m gate=$G"
