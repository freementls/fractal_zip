...
/**
 * Harmony tier B+C: siteinfo pack + text-region tokens (no full-blob, no PAQ).
 */
...
fwrite(STDERR, "[enwik8_harmony_text] encode start\n");
$t0 = microtime(true);
$fz = new fractal_zip();
...
file_put_contents($outJson, json_encode(array(
	'generated' => date('c'),
	'bench_profile' => 'world-record-harmony-text',
	'cases' => array(array(
		'label' => 'test_files109',
...
fwrite(STDERR, "[enwik8_harmony_text] fzc={$fzcBytes} B " . number_format($sec, 1) . "s → {$outJson}\n");
