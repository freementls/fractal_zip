...
/**
 * Scoreboard from a filled lifestyle run_benchmarks JSON.
 *
 * Goals (product bar):
 *   - Untied byte wins on every case (fzc sole-smallest vs gzip/7z/min-ext)
 *   - TOTAL zip_seconds < TOTAL fair min-ext winner compress wall (same gzip/7z/ext
 *     set as sole: time of the byte-minimum candidate; byte ties → min time)
 *
 * Usage: php benchmarks/analyze_lifestyle_full_table.php [path.json]
 */
$path = $argv[1] ?? (__DIR__ . '/.lifestyle_full_table.json');
if (!is_file($path)) {
...
echo "\n=== GOAL: untied bytes + beat fair min-ext encode wall ===\n";
$n = count($cases);
...
if ($totZip !== null && $totExt !== null) {
	$beat = $totZip < $totExt;
	printf(
		"TOTAL zip_s=%.2f  TOTAL fair_ext_s=%.2f  ratio=%.3f×  beat_fair_ext_s=%s\n",
		$totZip,
		$totExt,
...
$goalOk = ($n > 0 && $soleN === $n && count($losses) === 0 && count($tied) === 0
	&& $totZip !== null && $totExt !== null && $totZip < $totExt);
