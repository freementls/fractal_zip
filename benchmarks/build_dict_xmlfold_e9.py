#!/usr/bin/env python3
"""Build Profile B4 dictionary: post B0+XML_FOLD stream counts."""
...
CACHE = f"{ROOT}/benchmarks/.ladder_cache/enwik9_postxml_word_counts.tsv"
OUT_APPEND = f"{OUT_DIR}/english_xmlfold_e9.dic"
CAP = 44880
FOLD = [
    (b'<text xml:space="preserve">', bytes([6])),
    (b"</contributor>", bytes([7])),
    (b"<contributor>", bytes([8])),
    (b"</timestamp>", bytes([9])),
    (b"<timestamp>", bytes([0x0B])),
    (b"</revision>", bytes([0x0C])),
    (b"<revision>", bytes([0x0D])),
    (b"</title>", bytes([0x0E])),
    (b"<title>", bytes([0x0F])),
    (b"</page>", bytes([0x10])),
    (b"<page>", bytes([0x11])),
...
def main() -> None:
    ...
    for w in (b"quot", b"title", b"page", b"revision", b"contributor"):
        print(f"  postxml count[{w.decode()}]={counts.get(w, 0):,}", file=sys.stderr)
    cands = []
    for w, c in counts.items():
