...
/**
 * Same native-folder 7z wires as {@see benchmarks/fz_fz7_compact_folder_roundtrip_smoke.php} (raw / {@code N} / {@code FZ}), but loads only
 * {@code fractal_zip-undeep-unwrap.php} (must never be combined with {@code fractal_zip.php} in one process).
 *
 * Run: php benchmarks/fz_fz7_undeep_folder_roundtrip_smoke.php
 * Skips with exit 0 when 7z is missing.
 */

$repo = realpath(dirname(__DIR__)) ?: dirname(__DIR__);
require_once $repo . DIRECTORY_SEPARATOR . 'fractal_zip-undeep-unwrap.php';

if (fractal_zip::seven_zip_executable() === null) {
	fwrite(STDOUT, "SKIP fz_fz7_undeep_folder_roundtrip_smoke (7z not on PATH)\n");
	exit(0);
}

$tag = 'fz_fz7u_rt_' . bin2hex(random_bytes(6));
...
file_put_contents($src . DIRECTORY_SEPARATOR . 'hello.txt', "7z-undeep-a\n");
file_put_contents($src . DIRECTORY_SEPARATOR . 'sub' . DIRECTORY_SEPARATOR . 'deep.txt', "7z-undeep-b\n");
...
$expect = array('hello.txt' => "7z-undeep-a\n", 'sub/deep.txt' => "7z-undeep-b\n");
...
if ($runOpenAndVerify('raw_undeep', $native, '_ex_raw', $fzClean, $expect) !== 0) {
	exit(1);
}
...
if ($runOpenAndVerify('n1_undeep', $wireN, '_ex_n', $fzClean, $expect) !== 0) {
	exit(1);
}
...
if ($runOpenAndVerify('fz_legacy_undeep', $wireFz, '_ex_fz', $fzClean, $expect) !== 0) {
	exit(1);
}

fwrite(
	STDOUT,
	'OK fz_fz7_undeep_folder_roundtrip_smoke (raw=' . (string) strlen($native) . ' B, N=' . (string) strlen($wireN) . ' B, FZ=' . (string) strlen($wireFz) . " B).\n"
);
exit(0);
