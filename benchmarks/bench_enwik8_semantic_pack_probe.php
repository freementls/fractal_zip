...
/**
 * gzip-1 probe on 8 MiB enwik8 slice: raw vs semantic-pack.
 */
...
$path = $repo . DIRECTORY_SEPARATOR . 'benchmarks' . DIRECTORY_SEPARATOR . '.enwik8_semantic_pack_probe.json';
file_put_contents($path, json_encode($out, JSON_PRETTY_PRINT));
echo "semantic pack probe: delta=" . number_format($out['delta_bytes']) . " B (need >1MiB for strong signal)\n";
echo "  wrote {$path}\n";
