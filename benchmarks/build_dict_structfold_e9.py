#!/usr/bin/env python3
"""Build Profile B5 dictionary: post B0+STRUCT_FOLD stream counts.

Matches FXCM_ENTITY_FOLD + FXCM_STRUCT_FOLD. Block entity-only bodies and
wiki tokens that collapse into fold codes (quot; Category/Image/File as
link-prefix leftovers are still real words when not in [[Category: form).
"""
...
CACHE = f"{ROOT}/benchmarks/.ladder_cache/enwik9_poststruct_lite_word_counts.tsv"
OUT_APPEND = f"{OUT_DIR}/english_structfold_lite_e9.dic"
CAP = 44880
# Longest-first; must match preprocessor.cpp STRUCT_FOLD table + B0 entities.
FOLD = [
    (b"[[Wikipedia:", bytes([6])),
    (b"[[Category:", bytes([7])),
    (b"[[Image:", bytes([8])),
...
def main() -> None:
    ...
    for w in (b"quot", b"amp", b"lt", b"gt", b"category", b"image", b"file"):
        print(f"  poststruct_lite count[{w.decode()}]={counts.get(w, 0):,}", file=sys.stderr)
    ...
    print("top15 poststruct append:", [(w.decode(), c) for _, c, w in cands[:15]], file=sys.stderr)
    if b"quot" in chosen:
        raise SystemExit("BUG: quot leaked into append")
