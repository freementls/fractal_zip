...
function fractal_zip_image_pac_preprocess_literal_for_bundle(string $relPath, string $rawBytes): string {
	...
	if (!fractal_zip_content_format_policy_is_raster_member($relPath, $rawBytes)) {
		return $rawBytes;
	}
	$ext = strtolower((string) pathinfo($relPath, PATHINFO_EXTENSION));
	if (!isset(FRACTAL_ZIP_IMAGEPAC_RASTER_EXT[$ext])) {
		if (str_starts_with($rawBytes, "\xff\xd8\xff")) {
			$ext = 'jpg';
		} elseif (str_starts_with($rawBytes, "\x89PNG\r\n\x1a\n")) {
			$ext = 'png';
		} elseif (str_starts_with($rawBytes, 'GIF8')) {
			$ext = 'gif';
		} elseif (str_starts_with($rawBytes, 'BM')) {
			$ext = 'bmp';
		} elseif (strlen($rawBytes) >= 12 && substr($rawBytes, 0, 4) === 'RIFF' && substr($rawBytes, 8, 4) === 'WEBP') {
			$ext = 'webp';
		} else {
			$ext = '';
		}
	}
	if ($ext === '' || !isset(FRACTAL_ZIP_IMAGEPAC_RASTER_EXT[$ext])) {
		return $rawBytes;
	}
	...
	try {
		switch ($ext) {
			case 'png':
				...
				// Lifestyle midsize singles (metis.png / 81): level 4 + no filter
				// search + FZB+zstd soles soft xz (~36 KiB) — ALL_FILTERS was
				// ~0.25 s pure wall for identical bytes on this corpus.
				$pngLevel = 9;
				if (class_exists('fractal_zip', false)
					...
					$len = strlen($bytes);
					if ($len >= 256 * 1024 && $len <= 2 * 1024 * 1024) {
						$pngLevel = 4;
						$filters = defined('PNG_NO_FILTER') ? PNG_NO_FILTER : 0;
					}
				}
				imagepng($im, null, $pngLevel, $filters);
				$out = ob_get_clean();
				break;
			case 'gif':
