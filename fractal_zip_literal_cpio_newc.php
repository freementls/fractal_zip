...
/**
 * List file records from a newc cpio archive (stops at TRAILER!!!).
 *
 * @return list<array{name: string, data: string}>|null
 */
function fractal_zip_literal_cpio_newc_list_file_members(string $bytes): ?array {
	if ($bytes === '' || !fractal_zip_literal_cpio_newc_magic6_ok(substr($bytes, 0, 6))) {
		return null;
	}
	$out = array();
	$off = 0;
	$n = strlen($bytes);
	$cap = 256;
	$eCap = getenv('FRACTAL_ZIP_FOLDER_CPIO_MAX_MEMBERS');
	if ($eCap !== false && (int) $eCap > 0) {
		$cap = max(1, (int) $eCap);
	}
	while ($off + 110 <= $n && count($out) < $cap) {
		if (fractal_zip_literal_cpio_newc_record_is_trailer($bytes, $off) === true) {
			break;
		}
		$lay = fractal_zip_literal_cpio_newc_one_record_layout($bytes, $off);
		if ($lay === null) {
			break;
		}
		list($dStart, $fSize, , , $endRec) = $lay;
		$ns = fractal_zip_literal_cpio_newc_parse_hex8(substr($bytes, $off, 110), 11);
		if ($ns < 1) {
			break;
		}
		$rawName = substr($bytes, $off + 110, $ns);
		$name = (string) preg_replace('/\0.*/s', '', $rawName);
		$name = str_replace('\\', '/', $name);
		$name = ltrim($name, '/');
		if ($name !== '' && !str_contains($name, '..') && $fSize >= 0) {
			$out[] = array(
				'name' => $name,
				'data' => substr($bytes, $dStart, $fSize),
				'header_offset' => $off,
				'record_end' => $endRec,
			);
		}
