 ...
 * - {@code FRACTAL_ZIP_PEEL_JOBS=N} — max concurrent peel workers for multi-member {@see fractal_zip::collect_fractal_inner_peeler_variants_for_folder} ({@see parallel_member_peeler_views_collect}, {@see parallel_peeler_source_candidates_collect}). CLI unset ⇒ auto ({@see fractal_zip_parallel_runtime_fork_pool_workers}); {@code 1} or {@code 0} = serial; capped by {@code FRACTAL_ZIP_PIPELINE_JOBS}. Workers force {@code FRACTAL_ZIP_PEEL_JOBS=1} to avoid nested peel forks.
 * - {@code FRACTAL_ZIP_PEEL_GPU=1} — in peel workers, prefer {@code tools/gpu_substring} Rust helper ({@see fractal_zip_gpu_substring_peel_delegate_top_k}); CLI unset + Rust binary built ⇒ auto-on ({@see fractal_zip_parallel_runtime_apply_cli_defaults}).
 * - {@code FRACTAL_ZIP_PARALLEL_OFF=1} / {@code FRACTAL_ZIP_PARALLEL_PROBE=1} — disable integrated auto parallelism ({@see fractal_zip_parallel_runtime.php}).
 * - {@code FRACTAL_ZIP_PEEL_PRIORITY=size|format} — fork **start** order for peel jobs (default {@code size}): descending raw bytes; {@code format} adds content-profile difficulty tie-break ({@see peel_member_scheduling_weight}). Merge order stays path-sorted / source discovery order for wire-byte determinism.
 * - {@code FRACTAL_ZIP_PEEL_MIN_MEMBERS=N} — minimum folder members before peel fork pool (default 2).
 * - {@code FRACTAL_ZIP_FZBM_PATH_ORDER_PARALLEL=1} — fork pool over FZBM concat-order brotli/gzip proxy trials ({@see fractal_zip::literal_bundle_pick_fzbm_concat_key_order}); min candidates {@code FRACTAL_ZIP_FZBM_PATH_ORDER_PARALLEL_MIN} (default 8).
...
final class fractal_zip_encode_pipeline {
	...
	/** @var (callable(float, string, array): void)|null Host progress hook (e.g. fs compress bar). */
	private static $progressListener = null;

	/** @var float Last inner ratio emitted to progress listener (unmapped segment fallback). */
	private static $progressLastInnerRatio = 0.0;

	/** @var bool */
	private static $progressListenerActive = false;
	...
	/**
	 * Register a listener for {@see zip_folder} pipeline checkpoints (phase + segment labels).
	 * {@code $listener} receives (innerRatio 0..1, human detail).
	 */
	public static function set_progress_listener(?callable $listener): void {
		self::$progressListener = $listener;
		self::$progressListenerActive = $listener !== null;
		self::$progressLastInnerRatio = 0.0;
	}

	public static function clear_progress_listener(): void {
		self::$progressListener = null;
		self::$progressListenerActive = false;
		self::$progressLastInnerRatio = 0.0;
	}

	/** Map pipeline phase constants to progress within zip_folder (0..1). */
	private static function checkpoint_phase_ratio(int $phase): float {
	...
	/** Known zip_folder segment labels → inner progress (finer than phase checkpoints). */
	private static function checkpoint_segment_ratio(string $label): ?float {
		static $map = array(
			'zip_folder_legacy_markers_multipass_encode_collect_literals' => 0.30,
			'choose_smallest_phase4_encode_raw_and_literal_jobs' => 0.50,
			'choose_smallest_parallel_literal_outer_trials_fast' => 0.58,
			'choose_smallest_parallel_literal_outer_trials_full' => 0.66,
			'zip_folder_parallel_overlap_literal_wire_fast_lens' => 0.54,
			'zip_folder_parallel_wire_fast_outer_lens_fork_wait' => 0.70,
			'zip_folder_wire_fast_lens_pick_before_full_outer' => 0.76,
			'adaptive_compress_outer_fast_codec_tier' => 0.78,
			'adaptive_compress_prediction_fork_discard_finalize' => 0.56,
			'zip_folder_finalize_write_payload_trailer' => 0.90,
			'zip_folder_fzc_file_put_contents' => 0.97,
		);
		if (isset($map[$label])) {
			return (float) $map[$label];
		}
		foreach ($map as $prefix => $ratio) {
			if (str_starts_with($label, $prefix) || str_contains($label, $prefix)) {
				return (float) $ratio;
			}
		}
		if (str_contains($label, 'peeler') || str_contains($label, 'multipass') || str_contains($label, 'markers')) {
			return 0.28;
		}
		if (str_contains($label, 'literal') && str_contains($label, 'inner')) {
			return 0.44;
		}
		if (str_contains($label, 'native_compare')) {
			return 0.82;
		}
	...
	/** Human-readable compress bar text for zip_folder segment labels. */
	private static function checkpoint_segment_detail(string $label): string {
		static $exact = array(
			'zip_folder_legacy_markers_multipass_encode_collect_literals' => 'Peeling & multidiff passes…',
			'choose_smallest_phase4_encode_raw_and_literal_jobs' => 'Inner compression trials…',
			'choose_smallest_parallel_literal_outer_trials_fast' => 'Fast outer codec trials…',
			'choose_smallest_parallel_literal_outer_trials_full' => 'Full outer codec trials…',
			'zip_folder_parallel_overlap_literal_wire_fast_lens' => 'Comparing inner transforms…',
			'zip_folder_parallel_wire_fast_outer_lens_fork_wait' => 'Outer codec trials…',
			'zip_folder_wire_fast_lens_pick_before_full_outer' => 'Selecting best inner wire…',
			'adaptive_compress_outer_fast_codec_tier' => 'Outer: gzip / zstd / brotli…',
			'adaptive_compress_prediction_fork_discard_finalize' => 'Outer prediction…',
			'zip_folder_finalize_write_payload_trailer' => 'Finalizing .fz payload…',
			'zip_folder_fzc_file_put_contents' => 'Writing .fz file…',
		);
		if (isset($exact[$label])) {
			return $exact[$label];
		}
		if (str_contains($label, 'peeler') || str_contains($label, 'multipass') || str_contains($label, 'markers')) {
			return 'Peeling members…';
		}
		if (str_contains($label, 'literal') || str_contains($label, 'inner') || str_contains($label, 'choose_smallest')) {
			return 'Inner compression trials…';
		}
		if (str_contains($label, 'outer') || str_contains($label, 'adaptive_compress') || str_contains($label, 'wire')) {
			return 'Outer codec trials…';
		}
		if (str_contains($label, 'native_compare')) {
			return 'Comparing native archive codecs…';
		}

		return ucfirst(str_replace('_', ' ', $label));
	}

	/** adaptive_compress outer-step labels → zip_folder inner progress. */
	private static function checkpoint_outer_step_ratio(string $label): ?float {
		static $map = array(
			'after_gzip_baseline' => 0.64,
			'after_zstd_outer' => 0.68,
			'after_brotli_first_outer' => 0.72,
			'after_brotli_outer_complete' => 0.74,
			'after_fast_codec_merge' => 0.76,
			'before_outer_xz' => 0.77,
			'after_outer_xz' => 0.78,
			'before_outer_prediction_finalize_remainder' => 0.79,
			'after_outer_prediction_finalize_remainder' => 0.80,
			'before_slow_outer_sweep_gates' => 0.81,
			'after_slow_outer_sweep_gates' => 0.82,
			'before_outer_ratchet' => 0.83,
			'after_outer_ratchet' => 0.84,
			'before_brotli_full_outer' => 0.85,
			'after_brotli_full_outer' => 0.86,
			'before_outer_7z_huge_once' => 0.87,
			'after_outer_7z_huge_once' => 0.88,
			'before_outer_zpaq_predict_lane' => 0.86,
			'after_outer_zpaq_predict_lane' => 0.87,
			'before_outer_7z_predict_lane' => 0.86,
			'after_outer_7z_predict_lane' => 0.87,
			'before_outer_arc_predict_lane' => 0.86,
			'after_outer_arc_predict_lane' => 0.87,
			'after_parallel_slow_outer_wave_trials' => 0.89,
		);
		if (isset($map[$label])) {
			return (float) $map[$label];
		}
		if (str_starts_with($label, 'before_outer_') || str_starts_with($label, 'after_outer_')) {
			return 0.84;
		}
		if (str_contains($label, 'brotli')) {
			return 0.75;
		}
		if (str_contains($label, 'zstd')) {
			return 0.69;
		}
		if (str_contains($label, 'gzip')) {
			return 0.64;
		}
	...
	private static function checkpoint_outer_step_detail(string $label): string {
		static $exact = array(
			'after_gzip_baseline' => 'Outer: gzip baseline…',
			'before_zstd_outer' => 'Outer: zstd trial…',
			'after_zstd_outer' => 'Outer: zstd complete…',
			'after_brotli_first_outer' => 'Outer: brotli trial…',
			'after_brotli_outer_complete' => 'Outer: brotli complete…',
			'after_fast_codec_merge' => 'Outer: fast codecs merged…',
			'before_outer_xz' => 'Outer: xz trial…',
			'after_outer_xz' => 'Outer: xz complete…',
			'before_outer_prediction_finalize_remainder' => 'Outer prediction…',
			'after_outer_prediction_finalize_remainder' => 'Outer prediction complete…',
			'before_slow_outer_sweep_gates' => 'Outer: slow codec sweep…',
			'after_slow_outer_sweep_gates' => 'Outer: slow codec sweep…',
			'before_outer_ratchet' => 'Outer: ratchet tuning…',
			'after_outer_ratchet' => 'Outer: ratchet complete…',
			'before_brotli_full_outer' => 'Outer: full brotli…',
			'after_brotli_full_outer' => 'Outer: brotli complete…',
			'before_outer_7z_huge_once' => 'Outer: 7z trial…',
			'after_outer_7z_huge_once' => 'Outer: 7z complete…',
			'before_outer_zpaq_predict_lane' => 'Outer: zpaq trial…',
			'after_outer_zpaq_predict_lane' => 'Outer: zpaq complete…',
			'before_outer_7z_predict_lane' => 'Outer: 7z trial…',
			'after_outer_7z_predict_lane' => 'Outer: 7z complete…',
			'before_outer_arc_predict_lane' => 'Outer: arc trial…',
			'after_outer_arc_predict_lane' => 'Outer: arc complete…',
			'after_parallel_slow_outer_wave_trials' => 'Outer: slow codecs complete…',
		);
		if (isset($exact[$label])) {
			return $exact[$label];
		}
		if (str_contains($label, 'zpaq')) {
			return 'Outer: zpaq…';
		}
		if (str_contains($label, '7z')) {
			return 'Outer: 7z…';
		}
		if (str_contains($label, 'arc')) {
			return 'Outer: arc…';
		}
		if (str_contains($label, 'brotli')) {
			return 'Outer: brotli…';
		...
		if (str_contains($label, 'prediction')) {
	...
	/** Short UI label for pipeline phase checkpoints. */
	private static function checkpoint_phase_short_label(int $phase): string {
		if ($phase === self::PHASE_STREAM_REORDER) {
			return 'Phase 2.5';
		}
		switch ($phase) {
	...
	public static function progress_listener_active(): bool {
		return self::$progressListenerActive;
	}

	/** Host hook: {@see adaptive_compress} outer-step labels during {@see fractal_zip::zip_folder}. */
	public static function report_adaptive_outer_step_progress(string $label): void {
		if (!self::$progressListenerActive || $label === 'start' || $label === '') {
			return;
		}
		$ratio = self::checkpoint_outer_step_ratio($label);
		if ($ratio === null) {
			return;
		}
		self::emit_progress_checkpoint($ratio, self::checkpoint_outer_step_detail($label), array(
			'outer_step' => $label,
		));
	}
	 ...
	 */
	private static function emit_progress_checkpoint(float $innerRatio, string $detail, array $meta = array()): void {
		if (!self::$progressListenerActive || self::$progressListener === null) {
			return;
		}
		$innerRatio = max(0.0, min(1.0, $innerRatio));
		self::$progressLastInnerRatio = $innerRatio;
		$detail = trim($detail);
		if ($detail === '') {
			return;
		}
		(self::$progressListener)($innerRatio, $detail, $meta);
	}
	...
	/**
	 * Fork IPC JSON: compact; uses {@see bench_json_encode_try} (UTF-8 substitute when available).
	 *
	 * @param mixed $value
	 */
	private static function pipeline_ipc_json_put(string $path, $value): bool {
		$js = bench_json_encode_try($value, false);
		if ($js === null) {
			return false;
		}

		return @file_put_contents($path, $js) !== false;
	}

	/**
	 * @param mixed $value
	 */
	private static function pipeline_ipc_json_put_best_effort(string $path, $value): void {
		$js = bench_json_encode_try($value, false);
		if ($js !== null) {
	...
	public static function pipeline_outer_step_rollup_zip_folder_begin(): void {
		...
		if (self::$progressListenerActive) {
			self::$outerStepRollupSessionActive = true;
		}
	}
	...
	public static function pipeline_outer_step_rollup_zip_folder_segment(string $label): void {
		if (!self::$outerStepRollupSessionActive) {
			return;
		}
		...
		if (self::$progressListenerActive) {
			$segRatio = self::checkpoint_segment_ratio($label);
			if ($segRatio !== null) {
				self::emit_progress_checkpoint($segRatio, self::checkpoint_segment_detail($label), array(
					'segment' => $label,
				));
			} else {
				$fallback = min(0.96, self::$progressLastInnerRatio + 0.012);
				self::emit_progress_checkpoint($fallback, self::checkpoint_segment_detail($label), array(
					'segment' => $label,
				));
			}
		}
	...
	public static function pipeline_outer_step_rollup_zip_folder_end(): void {
		...
		$rollupEnabled = self::pipeline_outer_step_rollup_enabled();
	 ...
	 */
	public static function detect_cpu_worker_default(): int {
		return fractal_zip_parallel_runtime_cpu_count();
	}
	...
	public static function parallel_huge_once_arc_before_wave_trials(string $innerBlob, bool $hugeArcWaveGatePassed = false): ?array {
		...
		foreach (array($p0, $p1) as $idx => $cpid) {
			...
			$meta = bench_json_decode_file_assoc_try($jpath, 'encode_pipeline');
			if (!is_array($meta)) {
				continue;
			}
			if ((int) ($meta['ok'] ?? 0) !== 1) {
		...
		try {
			...
			if ($ji === 0) {
				...
				if ($seven === null || !fractal_zip::outer_skip_env_allows('FRACTAL_ZIP_SKIP_7Z')) {
					self::pipeline_ipc_json_put_best_effort($jp, array('ok' => 0, 'kind' => $kind));
			...
			if (!self::pipeline_ipc_json_put($jp, $meta)) {
				@unlink($bp);
				exit(1);
			}
			exit(0);
	...
	public static function parallel_zstd_fzb_16_and_15_trials(string $innerBlob, bool $forkAltWaveGatePassed = false): ?array {
		...
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'levels.json', array(16, 15))) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
	...
	private static function child_zstd_fzb_alt_levels_exit(string $td, int $ji, string $innerPath): void {
		...
		try {
			...
			$levels = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'levels.json', 'encode_pipeline levels.json', 512, 0, true, true);
			...
			if ($exe === null || !fractal_zip::outer_skip_env_allows('FRACTAL_ZIP_SKIP_ZSTD')) {
				self::pipeline_ipc_json_put_best_effort($jp, array('ok' => 0, 'level' => $lev));
	...
	public static function parallel_slow_outer_wave_trials(array $kinds, string $innerBlob, int $innerLen, int $bestLenBeforeWave, bool $fzbHuge, bool $slowWaveForkGatePassed = false): ?array {
		...
		if (!self::pipeline_ipc_json_put($ctxPath, $ctx)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
			return null;
		}
	...
	private static function child_slow_outer_wave_exit(string $td, int $ji, string $kind, string $innerPath, string $ctxPath): void {
		...
		try {
			$inner = (string) @file_get_contents($innerPath);
			$ctx = bench_json_decode_file_assoc_try($ctxPath, 'encode_pipeline slow_outer_wave ctx', 512, 0, true, true);
			if (!is_array($ctx)) {
				exit(1);
			}
	...
	private static function slow_outer_wave_write_meta_only(string $td, int $ji, string $kind, int $ok): void {
		$jp = $td . DIRECTORY_SEPARATOR . (string) $ji . '.json';
		self::pipeline_ipc_json_put_best_effort($jp, array('ok' => $ok, 'kind' => $kind));
	}
	...
	public static function parallel_brotli_lgwin_variant_trials(string $innerBlob, array $trials, bool $lgwinForkGatePassed = false): ?array {
		...
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'trials.json', $trials)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
	...
	public static function parallel_zpaq_outer_method_variant_trials(string $innerBlob, array $methFragments, bool $zpaqMethodForkGatePassed = false): ?array {
		...
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'candidates.json', $candsPlain)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
	...
	public static function parallel_folder_native_zpaq_method_variant_trials(string $boxAbs, array $methFragments, string $targetArg, string $prefix, string $zpaqExe): ?array {
		...
		if (@file_put_contents($td . DIRECTORY_SEPARATOR . 'box.txt', $boxAbs) === false
			|| !self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'candidates.json', array_values($methFragments))
	...
	private static function child_folder_native_zpaq_exit(string $td, int $ji): void {
		...
		try {
			...
			$cands = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'candidates.json', 'encode_pipeline candidates.json', 512, 0, true, true);
			...
			if ($zp === '') {
				self::pipeline_ipc_json_put_best_effort($jp, array('ok' => 0));
	...
	private static function child_brotli_lgwin_variant_exit(string $td, int $ji, string $innerPath): void {
		...
		try {
			$inner = (string) @file_get_contents($innerPath);
			$trials = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'trials.json', 'encode_pipeline trials.json', 512, 0, true, true);
	...
	public static function speculative_outer_finalize_huge_probe_brotli(?int $pid, ?string $td, bool $discard): ?string {
		...
		$meta = bench_json_decode_file_assoc_try($jp, 'encode_pipeline');
	...
	private static function speculative_outer_child_huge_probe_brotli_exit(string $td, string $innerBlob): void {
		...
		try {
			...
			$exe = fractal_zip::brotli_executable();
			if ($exe === null) {
				self::pipeline_ipc_json_put_best_effort($td . DIRECTORY_SEPARATOR . 'meta.json', array('ok' => 0));
				exit(3);
			}
			...
			if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'meta.json', $meta)) {
				@unlink($bp);
				exit(1);
			}
			exit(0);
	...
	public static function speculative_outer_prediction_begin(string $innerBlob, string $repoRoot, bool $overlapGateAlreadySatisfied = false): array {
		...
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'ctx.json', array('repoRoot' => $repoRoot))) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
	...
	private static function child_outer_xz_overlap_exit(string $td, string $innerPath): void {
		self::fork_child_prepare();
		try {
			$ctx = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'xz_ctx.json', 'encode_pipeline xz_ctx.json', 512, 0, true, true);
			...
			if (!self::pipeline_ipc_json_put($jp, array('ok' => 1, 'len' => strlen($blob)))) {
				@unlink($bp);
	...
	public static function speculative_outer_xz_fork_overlap_predict_begin(string $xzExe, string $innerBlob, int $xzLevel, ?float $timeoutSec, ?string $reuseTmpIn, bool $xzOverlapEnvGatePassed = false): array {
		...
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'xz_ctx.json', $ctx)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
	...
	private static function speculative_outer_prediction_child_exit(string $td, string $innerPath): void {
		try {
			$inner = (string) @file_get_contents($innerPath);
			$ctx = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'ctx.json', 'encode_pipeline outer_predict ctx.json', 512, 0, true, true);
	...
	public static function reorder_raw_members_for_locality(array $rawFilesByPath): array {
		...
		foreach ($out as $path => $bytes) {
			$b = fractal_zip_content_format_policy_content_profile_bucket((string) $path, (string) $bytes);
			if (!isset($byExt[$b])) {
				$byExt[$b] = array();
			}
		...
		self::html_trace_checkpoint(self::PHASE_STREAM_REORDER, 'member order: content-profile buckets + LPT by raw size within bucket');
	...
	public static function html_trace_checkpoint(int $phase, string $note): void {
		...
		if (self::$progressListenerActive) {
			self::emit_progress_checkpoint(
				self::checkpoint_phase_ratio($phase),
				self::checkpoint_phase_short_label($phase),
				array('pipeline_phase' => $phase)
			);
		}
	 ...
	 */
	public static function fork_child_prepare(): void {
		if (function_exists('fractal_zip_process_guard_fork_child_prepare')) {
			fractal_zip_process_guard_fork_child_prepare();
		}
	}

	public static function terminate_fork_children_and_cleanup(array $active, string $td): void {
		foreach (array_keys($active) as $cp) {
			...
			if (function_exists('fractal_zip_process_guard_untrack')) {
				fractal_zip_process_guard_untrack($cp);
			}
		}
		$st = 0;
		foreach (array_keys($active) as $cp) {
	...
	public static function unlink_temp_dir_contents(string $td): void {
		if (!is_dir($td)) {
			return;
		}
	...
	/** @return positive-int */
	public static function parallel_peel_jobs(): int {
		self::bootstrap_cli_parallel_defaults_if_cli();
		$e = getenv('FRACTAL_ZIP_PEEL_JOBS');
		if ($e === false || trim((string) $e) === '') {
			if (self::parallel_fork_allowed() && !fractal_zip_parallel_runtime_disabled() && !fractal_zip_parallel_runtime_probe_mode()) {
				return max(1, min(self::max_workers(), fractal_zip_parallel_runtime_fork_pool_workers()));
			}
			return 1;
		}
		$v = trim((string) $e);
		if ($v === '0' || !ctype_digit($v)) {
			return 1;
		}
		$n = (int) $v;
		if ($n <= 1) {
		...
		return max(1, min(self::max_workers(), $n));
	}

	public static function parallel_peel_pool_enabled(): bool {
		return self::parallel_peel_jobs() > 1
			&& self::pcntl_fork_supported_for_parallel_pool();
	}

	public static function parallel_peel_gpu_enabled(): bool {
		return fractal_zip_gpu_substring_peel_enabled();
	}

	public static function parallel_peel_min_members(): int {
		$e = getenv('FRACTAL_ZIP_PEEL_MIN_MEMBERS');
		if ($e === false || trim((string) $e) === '' || !ctype_digit(trim((string) $e))) {
			return 2;
		}
		return max(2, (int) trim((string) $e));
	}

	public static function parallel_peel_priority_mode(): string {
		$e = getenv('FRACTAL_ZIP_PEEL_PRIORITY');
		if ($e === false || trim((string) $e) === '') {
			return 'size';
		}
		$m = strtolower(trim((string) $e));
		return $m === 'format' ? 'format' : 'size';
	}

	/**
	 * Scheduling weight for peel fork starts (LPT). Higher = start sooner.
	 */
	public static function peel_member_scheduling_weight(string $path, string $bytes): float {
		$size = (float) strlen($bytes);
		if (self::parallel_peel_priority_mode() !== 'format') {
			return $size;
		}
		$bucket = fractal_zip_content_format_policy_content_profile_bucket($path, $bytes);
		$difficulty = 1.0;
		if (fractal_zip_content_format_policy_is_raster_member($path, $bytes)) {
			$difficulty = 4.0;
		} elseif (fractal_zip_content_format_policy_is_pdf_container($path, $bytes)) {
			$difficulty = 3.5;
		} elseif (!fractal_zip_content_format_policy_member_is_textish($path, $bytes)) {
			$difficulty = 2.5;
		} elseif ($bucket === 'xml' || $bucket === 'html') {
			$difficulty = 2.0;
		}
	...
	/**
	 * @param list<array{source: string, bytes: string, restore: string|array}> $views
	 * @return list<array<string, mixed>>
	 */
	private static function peel_ipc_serialize_views(array $views): array {
		$out = array();
		foreach ($views as $v) {
			$restore = $v['restore'] ?? '';
			if (is_array($restore)) {
				$r = array('kind' => 'array', 'payload' => $restore);
			} else {
				$r = array('kind' => 'string', 'payload' => base64_encode((string) $restore));
			}
			$out[] = array(
				'source' => (string) ($v['source'] ?? ''),
				'bytes' => base64_encode((string) ($v['bytes'] ?? '')),
				'restore' => $r,
			);
		}
		return $out;
	 ...
	 * @return list<array{source: string, bytes: string, restore: string|array}>
	 */
	private static function peel_ipc_deserialize_views(array $rows): array {
		$out = array();
		foreach ($rows as $row) {
			if (!is_array($row)) {
				continue;
			}
			$bytes = base64_decode((string) ($row['bytes'] ?? ''), true);
			if ($bytes === false) {
				continue;
			}
			$restoreRow = $row['restore'] ?? null;
			$restore = '';
			if (is_array($restoreRow) && isset($restoreRow['kind'])) {
				if ($restoreRow['kind'] === 'array' && isset($restoreRow['payload']) && is_array($restoreRow['payload'])) {
					$restore = $restoreRow['payload'];
				} elseif ($restoreRow['kind'] === 'string' && isset($restoreRow['payload'])) {
					$decoded = base64_decode((string) $restoreRow['payload'], true);
					$restore = $decoded === false ? '' : $decoded;
				}
			}
			$out[] = array(
	...
	/**
	 * JSON-safe candidate rows for fork IPC. Candidate 'inner' is a binary container
	 * wire and restore specs can hold exact disk bytes; raw JSON with
	 * JSON_INVALID_UTF8_SUBSTITUTE silently mangles them into U+FFFD.
	 *
	 * @param list<array<string, mixed>> $cands
	 * @return list<array<string, mixed>>
	 */
	private static function peel_ipc_serialize_candidates(array $cands): array {
		$out = array();
		foreach ($cands as $c) {
			if (!is_array($c)) {
				continue;
			...
			$restoreEnc = array();
			foreach ((isset($c['restore']) && is_array($c['restore']) ? $c['restore'] : array()) as $p => $spec) {
				if (is_array($spec)) {
					$restoreEnc[(string) $p] = array('kind' => 'array', 'payload' => $spec);
				} else {
					$restoreEnc[(string) $p] = array('kind' => 'string', 'payload' => base64_encode((string) $spec));
				}
			}
			$out[] = array(
				'inner' => base64_encode((string) ($c['inner'] ?? '')),
				'tag' => (string) ($c['tag'] ?? ''),
				'lane' => (string) ($c['lane'] ?? ''),
				'source' => (string) ($c['source'] ?? ''),
				'restore' => $restoreEnc,
				'cost_hint' => (int) ($c['cost_hint'] ?? 0),
			);
		}
		return $out;
	}
	 ...
	 * @return list<array<string, mixed>>
	 */
	private static function peel_ipc_deserialize_candidates(array $rows): array {
		$out = array();
		foreach ($rows as $row) {
			if (!is_array($row)) {
				continue;
			}
			$inner = base64_decode((string) ($row['inner'] ?? ''), true);
			if ($inner === false || $inner === '') {
				continue;
			}
			$restore = array();
			foreach ((isset($row['restore']) && is_array($row['restore']) ? $row['restore'] : array()) as $p => $spec) {
				if (!is_array($spec) || !isset($spec['kind'])) {
					continue;
				}
				if ($spec['kind'] === 'array' && isset($spec['payload']) && is_array($spec['payload'])) {
					$restore[(string) $p] = $spec['payload'];
				} elseif ($spec['kind'] === 'string' && isset($spec['payload'])) {
					$d = base64_decode((string) $spec['payload'], true);
					if ($d !== false) {
						$restore[(string) $p] = $d;
					}
				}
			...
			$out[] = array(
				...
				'source' => (string) ($row['source'] ?? ''),
				'restore' => $restore,
				'cost_hint' => max(0, (int) ($row['cost_hint'] ?? 0)),
			);
		}
		return $out;
	}
	...
	/**
	 * Parallel per-member peel views. Returns null to fall back to serial.
	 *
	 * @param array<string, string> $rawFilesByPath path-sorted member map
	 * @return array{memberViews: array<string, array<string, array{source: string, bytes: string, restore: string|array}>>, sourceOrder: list<string>, sources: array<string, true>}|null
	 */
	public static function parallel_member_peeler_views_collect(fractal_zip $fz, array $rawFilesByPath): ?array {
		if (!self::parallel_peel_pool_enabled() || count($rawFilesByPath) < self::parallel_peel_min_members()) {
			return null;
		}
		...
		$td = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'fz_peel_mem_' . bin2hex($rnd);
		if (!@mkdir($td, 0700, true)) {
			return null;
		}
		$manifest = array();
		foreach ($paths as $i => $path) {
			$bp = $td . DIRECTORY_SEPARATOR . (string) $i . '.bin';
			if (@file_put_contents($bp, (string) $rawFilesByPath[$path]) === false) {
				self::unlink_temp_dir_contents($td);
				@rmdir($td);
				return null;
			}
			$manifest[] = array('ji' => $i, 'path' => (string) $path);
		}
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'manifest.json', $manifest)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
			return null;
		}
		...
		usort(
			$forkOrder,
			static function (int $a, int $b) use ($paths, $rawFilesByPath): int {
				$pa = $paths[$a];
				$pb = $paths[$b];
				$wa = self::peel_member_scheduling_weight($pa, (string) $rawFilesByPath[$pa]);
				$wb = self::peel_member_scheduling_weight($pb, (string) $rawFilesByPath[$pb]);
				if ($wa !== $wb) {
					return $wa < $wb ? 1 : -1;
				}
				return $pa <=> $pb;
		...
		$workers = self::parallel_peel_jobs();
		$fp = 0;
		$active = array();
		$viewsByJi = array_fill(0, $n, null);
		while ($fp < $n || count($active) > 0) {
			while (count($active) < $workers && $fp < $n) {
				$ji = $forkOrder[$fp++];
				$pid = pcntl_fork();
				...
				if ($pid === 0) {
					self::child_peel_member_views_exit($td, $ji);
				}
				$active[$pid] = $ji;
			}
			if (count($active) === 0) {
			...
			$meta = bench_json_decode_file_assoc_try($jpath, 'encode_pipeline peel member');
			if (!is_array($meta) || (int) ($meta['ok'] ?? 0) !== 1 || !isset($meta['views']) || !is_array($meta['views'])) {
				continue;
			}
			$viewsByJi[$ji] = self::peel_ipc_deserialize_views($meta['views']);
		}
		self::unlink_temp_dir_contents($td);
		@rmdir($td);
		$memberViews = array();
		...
		foreach ($paths as $i => $path) {
			$views = $viewsByJi[$i];
			if (!is_array($views)) {
				$views = $fz->fractal_inner_member_peeler_views($path, (string) $rawFilesByPath[$path]);
			}
			$p = (string) $path;
			if (!isset($memberViews[$p])) {
				$memberViews[$p] = array();
			}
		...
		self::html_trace_checkpoint(self::PHASE_UNPEEL, 'parallel member peeler views: ' . (string) $n . ' members, ' . (string) count($sourceOrder) . ' sources');
		return array(
	...
	/**
	 * Parallel per-source peeled inner candidate discovery. Returns null to fall back to serial per-source loop.
	 *
	 * @param array<string, string> $base
	 * @param array<string, array<string, array{source: string, bytes: string, restore: string|array}>> $memberViews
	 * @param list<string> $sourceOrder
	 * @return array<string, list<array<string, mixed>>>|null map source => candidates
	 */
	public static function parallel_peeler_source_candidates_collect(
		fractal_zip $fz,
		array $base,
		array $memberViews,
		array $sourceOrder,
		...
		if (!self::parallel_peel_pool_enabled() || count($sourceOrder) < 2) {
			return null;
		}
		$n = count($sourceOrder);
		try {
		...
		$td = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'fz_peel_src_' . bin2hex($rnd);
		if (!@mkdir($td, 0700, true)) {
			return null;
		}
		$jobs = array();
		foreach ($sourceOrder as $si => $source) {
			$files = $base;
			$restore = array();
			$totalView = 0;
			foreach ($base as $p => $disk) {
				$totalView += strlen((string) $disk);
			}
			foreach ($base as $p => $disk) {
			...
			if ($restore === array() || ($maxTotal > 0 && $totalView > $maxTotal)) {
				$jobs[] = array('skip' => true, 'source' => (string) $source);
				continue;
			}
			...
			foreach ($files as $p => $bytes) {
				$filesEnc[(string) $p] = base64_encode((string) $bytes);
			}
			$restoreEnc = array();
			foreach ($restore as $p => $spec) {
				if (is_array($spec)) {
					$restoreEnc[(string) $p] = array('kind' => 'array', 'payload' => $spec);
				} else {
					$restoreEnc[(string) $p] = array('kind' => 'string', 'payload' => base64_encode((string) $spec));
			...
			$jobs[] = array(
				...
				'files' => $filesEnc,
				'restore' => $restoreEnc,
				'typed_restore_only' => $typedRestoreOnly,
				'total_view' => $totalView,
			);
		}
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'jobs.json', $jobs)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
			return null;
		}
		$ctx = array(
			'runGrammarMaxRawBytes' => $runGrammarMaxRawBytes,
			'operatorMaxRawBytes' => $operatorMaxRawBytes,
		);
		if (!self::pipeline_ipc_json_put($td . DIRECTORY_SEPARATOR . 'ctx.json', $ctx)) {
			self::unlink_temp_dir_contents($td);
			@rmdir($td);
			return null;
		}
		...
		usort(
			$forkOrder,
			static function (int $a, int $b) use ($jobs): int {
				$ja = $jobs[$a] ?? array();
				$jb = $jobs[$b] ?? array();
				if (!empty($ja['skip']) && !empty($jb['skip'])) {
					return strcmp((string) ($ja['source'] ?? ''), (string) ($jb['source'] ?? ''));
				}
				if (!empty($ja['skip'])) {
					return 1;
				}
				...
				$wa = (float) ($ja['total_view'] ?? 0);
				$wb = (float) ($jb['total_view'] ?? 0);
				if ($wa !== $wb) {
					return $wa < $wb ? 1 : -1;
				}
				return strcmp((string) ($ja['source'] ?? ''), (string) ($jb['source'] ?? ''));
		...
		while ($fp < $n || count($active) > 0) {
			while (count($active) < $workers && $fp < $n) {
				...
				if ($pid === 0) {
					self::child_peel_source_candidates_exit($td, $ji);
				}
				$active[$pid] = $ji;
			}
			if (count($active) === 0) {
			...
			$meta = bench_json_decode_file_assoc_try($jpath, 'encode_pipeline peel source');
			if (!is_array($meta) || (int) ($meta['ok'] ?? 0) !== 1) {
				continue;
			}
			$candBySi[$ji] = isset($meta['candidates']) && is_array($meta['candidates'])
				? self::peel_ipc_deserialize_candidates($meta['candidates'])
				: array();
		}
		self::unlink_temp_dir_contents($td);
		@rmdir($td);
		...
		foreach ($sourceOrder as $si => $source) {
			...
			$out[(string) $source] = $cands;
		}
		self::html_trace_checkpoint(self::PHASE_UNPEEL, 'parallel peeler source candidates: ' . (string) count($sourceOrder) . ' sources');
		return $out;
	}

	private static function child_peel_member_views_exit(string $td, int $ji): void {
		self::fork_child_prepare();
		try {
			putenv('FRACTAL_ZIP_PEEL_JOBS=1');
			if (self::parallel_peel_gpu_enabled()) {
				putenv('FRACTAL_ZIP_PEEL_GPU=1');
			}
			$manifest = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'manifest.json', 'peel manifest', 512, 0, true, true);
			if (!is_array($manifest) || !isset($manifest[$ji]) || !is_array($manifest[$ji])) {
				exit(1);
			}
			$path = (string) ($manifest[$ji]['path'] ?? '');
			$bp = $td . DIRECTORY_SEPARATOR . (string) $ji . '.bin';
			$disk = is_file($bp) ? (string) file_get_contents($bp) : '';
			if ($path === '' || $disk === '') {
				exit(1);
			}
			$fz = new fractal_zip(256, false, false, null, false);
			$views = $fz->fractal_inner_member_peeler_views($path, $disk);
			$jp = $td . DIRECTORY_SEPARATOR . (string) $ji . '.json';
			$meta = array(
				'ok' => 1,
				'views' => self::peel_ipc_serialize_views($views),
			);
			if (!self::pipeline_ipc_json_put($jp, $meta)) {
				exit(1);
			}
	...
	private static function child_peel_source_candidates_exit(string $td, int $ji): void {
		try {
			putenv('FRACTAL_ZIP_PEEL_JOBS=1');
			if (self::parallel_peel_gpu_enabled()) {
				putenv('FRACTAL_ZIP_PEEL_GPU=1');
			}
			require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'fractal_zip_inner.php';
			$jobs = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'jobs.json', 'peel jobs', 512, 0, true, true);
			$ctx = bench_json_decode_file_assoc_try($td . DIRECTORY_SEPARATOR . 'ctx.json', 'peel ctx', 32, 0, true, true);
			if (!is_array($jobs) || !isset($jobs[$ji]) || !is_array($jobs[$ji]) || !is_array($ctx)) {
				exit(1);
			}
			$job = $jobs[$ji];
			$jp = $td . DIRECTORY_SEPARATOR . (string) $ji . '.json';
			if (!empty($job['skip'])) {
				self::pipeline_ipc_json_put_best_effort($jp, array('ok' => 1, 'candidates' => array()));
				exit(0);
			}
			$source = (string) ($job['source'] ?? '');
			$files = array();
			foreach (($job['files'] ?? array()) as $p => $b64) {
				$decoded = base64_decode((string) $b64, true);
				if ($decoded !== false) {
					$files[(string) $p] = $decoded;
				}
			}
			$restore = array();
			foreach (($job['restore'] ?? array()) as $p => $spec) {
				if (!is_array($spec) || !isset($spec['kind'])) {
					continue;
				}
				if ($spec['kind'] === 'array' && isset($spec['payload']) && is_array($spec['payload'])) {
			...
			$fz = new fractal_zip(256, false, false, null, false);
			$candidates = array();
			if (!empty($job['typed_restore_only'])) {
				$escaped = array();
				foreach ($files as $p => $bytes) {
					$escaped[(string) $p] = $fz->escape_literal_for_storage((string) $bytes);
				}
				$candidates[] = fractal_zip_candidate_from_inner(
					$fz->encode_container_payload($escaped, ''),
					'peeler_literal:' . $source,
					'literal',
					'peeler_literal:' . $source,
					$restore,
					(int) ($job['total_view'] ?? 0)
				);
			} else {
				$candidates = fractal_zip_inner_candidates_for_peeled_folder(
					...
					$files,
					$restore,
					(int) ($ctx['runGrammarMaxRawBytes'] ?? 0),
					(int) ($ctx['operatorMaxRawBytes'] ?? 0),
					$source
				);
			}
			if (!self::pipeline_ipc_json_put($jp, array('ok' => 1, 'candidates' => self::peel_ipc_serialize_candidates($candidates)))) {
				exit(1);
			}
			exit(0);
		} catch (\Throwable $e) {
