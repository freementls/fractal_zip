...
**I challenge you to beat this compression!**

**Web demos:** [Compress to `.fz`](https://freement.cloud/fractal_zip/examples/fzc_compress.php) · [Extract `.fz`](https://freement.cloud/fractal_zip/examples/fzc_extract.php) (same nested-squares **fractal_zip** icon as FS uses for `.fz` — `fs-icon-fractal`; deploy `examples/favicon.ico` to the vhost document root if you want `/favicon.ico` site-wide)

**Desktop (v0.1):** native window app (`fz-desktop`) that mirrors the web demos — build with `desktop/scripts/package.sh` or run `desktop/scripts/run_dev.sh`; see [`docs/DESKTOP.md`](docs/DESKTOP.md). Publish release zips only after a functionality milestone (not yet).

Please avoid enormous uploads on the shared demos; run serious workloads locally. **Upload vs extract caps (demos):** staged compress totals honor **`FZC_WEB_MAX_UPLOAD_BYTES`** (default **8 MiB**); the extract page caps one **`.fz`** at **`FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES`** when set, otherwise **10×** that upload cap (default **80 MiB** extract vs **8 MiB** tree staging). See **`examples/fzc_web_shared.php`** and the footers on **`examples/fzc_compress.php`** / **`fzc_extract.php`**. **Local compress vs live extract:** if the web UI errors on zpaq/7z/arc but CLI works locally, the server is missing tools — **`docs/WEB_LOCAL_PARITY.md`**, **`scripts/fzc_parity_diagnose.sh`**, **`scripts/fzc_parity_gate.sh`** (pre-deploy), **`scripts/fzc_live_setup_check.sh`** (full live checklist), **`examples/fzc_capability_report.php`** (`?json=1` or plain text). For hard fail-fast on production drift, set **`FZC_WEB_ENFORCE_EXTRACT_COMPAT=1`** on the web host. **Web FS / ops:** `docs/FRACTAL_ZIP_READINESS.md` (reads, presets, job GC, **`ETag`**, **`Range`**, CORS). Bridge: `examples/fractal_zip_web_fs_bridge.php`. GC: `php examples/fzc_web_gc.php`. Regression: `./scripts/fractal_zip_web_smoke.sh`.
...
Multi-corpus **`--jobs`**, compressor threading, and tiered **`--bench-profile`** (**`large-bytes`**, …): **`benchmarks/LARGE_CORPUS_SPEED.md`** · scripted **`large-bytes`**: **`bash benchmarks/run_large_corpus_bytes_push.sh`**. Default **`benchmarks/.last_bench.json`** is **gitignored**; use **`--out-json=`** to keep a copy (**JSON machine output** section in that doc).
...
**fractal_zip** is a **PHP library** and **CLI** for a **lossless folder archive** format (`.fz`): a fractal-style **inner** container (`FZC1` / `FZC2`, literal bundles, optional PAC lanes, optional FZCD merged FLAC, …) plus an **adaptive outer** wrapper that competes **gzip‑9**, **7‑Zip**, **zstd**, **brotli**, **Arc**, **zpaq**, etc., keeping the smallest outer blob. It treats **directory trees as first-class data**, not only concatenated tar streams.

### Peel vs fractal_zip (extraction vs compression)

| | **[`/srv/http/peel`](../peel/)** | **fractal_zip (this repo)** |
|--|----------------------------------|-----------------------------|
| Role | Dragon Unpacker–style **peeling** | **Compression** + semantic peel for ratio |
| List / extract archives | Native handlers (WAD, PAK, GRP, SG `.dat`, …) + fractal_zip fallback | `.fz` + generic containers via web-fs |
| Nested unpack | `peel unpack` | — |
| Identify files | `peel probe` + `file_types` | Uses `file_types` for scheduling |
| Folder → `.fz` | `peel compress` → delegates here | `fractal_zip_cli.php zip` |
| Single-stream codecs | Identified via merged registry | `fractal_zip_literal_pac_registry.php` |

Use **peel** for extraction workflows; use **fractal_zip** directly when tuning encode env vars or running benchmarks.

---

## Documented bytes wins (vs strong baselines)

Unless noted, a **bytes win** means **`.fz` is ≤ the smallest of**: gzip‑9 **tarball**, **7z directory** archive, and the **min‑ext** tournament (Arc / zstd / brotli / xz / zpaq / …) from `benchmarks/run_benchmarks.php`. Full rows, dates, and commands live in **`benchmarks/BYTES_WIN_TRACKER.md`** (source of truth for public claims). That ledger currently includes pasted **`verify_ok: yes`** wins (or ties) for **`test_files2, 4, 10, 11, 13, 28, 29, 35, 49, 52, 53, 55_sample, 55, 60, 56_sample, 57, 114`** (plus **`test_files56`** with semantic **`verify_ok: false`** — see tracker notes). The stratified **`test_files69`** corpus is exercised in the driver; **paste a pinned JSON row** into the tracker when you publish fresh numbers.

### Standout results (representative)

| Area | Corpus | Why it matters |
|------|--------|----------------|
| **Huge real site tree** | **`test_files55`** (~200 MiB raw HTML) | `.fz` **beats** gzip‑9, 7z dir, and **min‑ext Arc** on recorded snapshot (`verify_ok` **yes** in tracker). Encoding is **slow** (benchmark order ~25 min `.fz` vs ~9 s gzip component — see tracker JSON). |
| **Arc‑native integration** | **`test_files57`** (~9 MiB PHP/HTML mirror) | When Arc wins min‑ext, `.fz` can store **raw FreeArc bytes** (passthrough) so bytes **match** the best external Arc archive (`verify_ok` **yes**). |
| **Mixed rasters + folders** | **`test_files61`** (~4.8 MiB raw) | **`run_benchmarks.php`** snapshots often show `.fz` **far below** naive gzip‑9 / strong min‑ext brotli on the same corpus (outer codec varies). Add a row to **`benchmarks/BYTES_WIN_TRACKER.md`** when you lock a reproducible command. PAC / semantic image modes may trade **SHA1** for **pixel** equality — check **`verify_ok`**. |
| **FLAC + sidecars (fast loop)** | **`test_files60`** | With **`FRACTAL_ZIP_FLACPAC` unset** (default), FLAC members stay **bit‑identical**; `.fz` edges min‑ext **zstd** on recorded snapshot. **`FRACTAL_ZIP_FLACPAC=1`** enables merged PCM FZCD for smaller `.fz` at the cost of SHA1 verify until a semantic FLAC comparator exists. |
| **Stratified ≤5 MiB mix** | **`test_files69`** | Text, HTML, phpinfo, rasters, FLAC+m3u, SC2 path, gzip‑peel dupes — README grid shows `.fz` vs strong baselines (refresh row in tracker when you publish new JSON). |
| **Synthetic / peel stress** | **`test_files62`** | Tree of `.gz` members exercises gzip peel + raw‑tier wire dual (`verify_ok` **yes** in tracker). |
| **Large single BMP** | **`test_files35`** | Photo‑stress raster; tracker lists `.fz` vs **xz** min‑ext (`verify_ok` **yes**). |
| **Micro‑many‑files** | **`test_files50`–`53`, etc.** | Hundreds of tiny files / CSV — documented wins or ties in tracker and grid. |
| **HTML subset of `test_files55`** | **`test_files55_sample`** (~26 MiB raw, every‑8th slice) | Full fractal path + unified stream; `.fz` beats baselines on tracker snapshot. |
...
### Public general-purpose lakes (`test_files202`–`210`)

Nine ~**3 GiB** domain lakes from **public** sources only (FOSS game data, Blender open movies, Digital Corpora govdocs PDFs, COCO photos, kernel/GCC/LLVM source trees, Debian `.deb`s, NYC TLC Parquet, GGUF/ONNX weights, already-compressed release tarballs). They are **gitignored** — materialize locally; **do not commit or push** the trees or `benchmarks/.gp_corpus_cache/`. Once built, they appear in **default** bench discovery (absent dirs stay absent). Prefer `--large` for honest heavy-folder bytes. Gold comparable TOTAL:

```bash
php benchmarks/build_test_files202_210_gp.php --force
php benchmarks/report_gp_corpora_coverage.php
./benchmarks/run_lifestyle_full_table.sh
# equivalent: FRACTAL_ZIP_BENCH_MEMORY_LIMIT=8G php benchmarks/run_benchmarks.php --large --json --out-json=benchmarks/.lifestyle_full_table.json
```


Small type-census smoke (synthetic fixtures, committed when present): **`test_files201`** via `php benchmarks/build_test_files201_web_census.php`.
...
- **Wall time:** `.fz` encode is usually **far slower** than gzip / zstd / often 7z on directory baselines; sometimes **competitive with or faster than the full min‑ext tournament wall** on a given corpus — see **`benchmarks/LARGE_CORPUS_SPEED.md`** and **`benchmarks/PARALLELISM.md`** (`--jobs`, threading env, **`--bench-profile`** tiers including **`large-bytes`** and **`bash benchmarks/run_large_corpus_bytes_push.sh`** for a scripted bytes‑push preset).
...
./scripts/fz zip path/to/folder          # → path/to/folder.fz
./scripts/fz extract path/to/archive.fz [output_dir]
./scripts/fz member-list path/to/archive.fz
./scripts/fz member-read path/to/archive.fz inner/path.txt -
./scripts/fz inspect path/to/archive.fz
./scripts/fz help
```
...
php fractal_zip_cli.php extract path/to/archive.fz [output_directory]
php fractal_zip_cli.php member-list path/to/archive.fz
php fractal_zip_cli.php member-read path/to/archive.fz path/inside.bin [out]
...
bash benchmarks/run_large_corpus_bytes_push.sh --only=test_files55_sample --large --no-case-timeout --json
```

- **`benchmarks/BYTES_WIN_TRACKER.md`** — protocol + pasted JSON rows for public wins.  
- **`benchmarks/ABLATION_LEDGER.md`** — progressive promote/block gate with soft closeness beside `verify_ok` (`php benchmarks/ablation_ledger.php --demo`; or `run_benchmarks.php --ablation-ledger`).  
- **`benchmarks/LARGE_CORPUS_SPEED.md`** — `--jobs`, threading, **`large-fast` / `large-balanced` / `large-bytes`**, gzip‑fast vs **`--large`**, baseline cache hygiene; **`bash benchmarks/run_large_corpus_bytes_push.sh`**.  
- **`benchmarks/PARALLELISM.md`** — env reference for fair compressor threading.  
- **`php benchmarks/report_bytes_wins.php`** — summarize wins from JSON; **`--compress-time-audit`** flags slow `.fz` encode vs baselines.  
- **`php benchmarks/verify_fzc_bytes_winner_each_corpus.php`** — assert `.fz` among byte winners across discovery; heavy-list corpora with raw ≥ **128 MiB** are verified with **`run_benchmarks.php --large`** automatically (**`bench_corpora_should_pass_large_to_run_benchmarks()`** in **`bench_predict_outer_encode.php`**; ratio parity with **`BYTES_WIN_TRACKER.md`**). On **`run_benchmarks.php --json`**, **`--no-verify`** records **`verify_ok` / `verify_mismatch_files` as `null`** (skipped), not false / zero.
...
├── LOM/
│   └── O.php
└── files/                ← physical blobs and `.fz` paths relative to this root
```
...
Different methodology than the **folder + min‑ext** grid above (old WinRAR / FreeArc / 7‑Zip vs early `.fz` on a few single files). Digits are **not** comparable to current `run_benchmarks.php` rows unless you reproduce the **same OS, builds, and bytes**.
...
## Native zpaq extract cache (FFS integration)

When `FILES_ROOT` is set (FFS does this via `ffs.files.php`), native folder zpaq extracts cache under **`{FILES_ROOT}derived/`**:

- **Tree cache:** `derived/fz-native-extract-cache/fzpa/{digest40}/members/` — fast `open_container`
- **Manifest:** `derived/fzpa/{digest40}/members.json` — member relpath → content sha256 in `{FILES_ROOT}{hash}`

Digest key is **sha256(raw zpaq arc)**, not the `FZPA\x01` wrapper. Env: `FRACTAL_ZIP_NATIVE_EXTRACT_CACHE`, `FRACTAL_ZIP_NATIVE_EXTRACT_CACHE_DIR`, `FRACTAL_ZIP_NATIVE_EXTRACT_MANIFEST`. Details: **`docs/NATIVE_EXTRACT_CACHE.md`**.
