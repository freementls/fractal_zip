 ...
 * Optional metastruct merge: FRACTAL_ZIP_METastruct=1 (see fractal_zip_metastruct.php / FZMS descriptor).
 *
 * Design notes:
...
function fractal_zip_marker_config_equals(array $a, array $b): bool {
	$ja = bench_json_encode_fingerprint_try(fractal_zip_marker_normalize_for_compare($a), 0);
	$jb = bench_json_encode_fingerprint_try(fractal_zip_marker_normalize_for_compare($b), 0);
...
function fractal_zip_maybe_apply_adaptive_markers(fractal_zip $host, string $dir): void {
	...
	$descriptor = null;
	if (fractal_zip_metastruct_env_enabled()) {
		require_once __DIR__ . DIRECTORY_SEPARATOR . 'fractal_zip_metastruct.php';
		list($proposed, $descriptor) = fractal_zip_metastruct_marker_propose($dir, $sample, $host->multipass);
		fractal_zip::$last_metastruct_descriptor = $descriptor;
	} else {
	...
	if (fractal_zip_marker_config_equals($proposed, $defaults)) {
		if (is_array($descriptor)) {
			$descriptor['applied'] = false;
			$descriptor['probe'] = array('skipped' => 'proposed_equals_defaults');
			fractal_zip::$last_metastruct_descriptor = $descriptor;
		}
		return;
	}
	...
	if (is_array($descriptor)) {
		$descriptor['applied'] = $applied;
		$descriptor['probe'] = array(
			'default_fractal_leg_bytes' => $dBytes,
			'proposed_fractal_leg_bytes' => $pBytes,
			'delta' => $pBytes - $dBytes,
		);
		fractal_zip::$last_metastruct_descriptor = $descriptor;
	}
