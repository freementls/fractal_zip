...
/**
 * Example: extract all members from a `.fz` archive (fractal_zip `open_container`).
 *
 * - **CLI:** `php fzc_extract.php <file.fz> [output_directory]` — same behavior as before.
 * - **Web:** drag-and-drop or pick a `.fz`, upload progress, then links under `web_jobs/{job}/staging/`. Download via `?job=&dl=staging/...`.
 *
 ...
 *
 * **Local vs live extract:** if compression worked locally but this page errors on zpaq/7z/arc, the server is missing tools — see `docs/WEB_LOCAL_PARITY.md` and `examples/fzc_capability_report.php`.
 *
 * **Storage roadmap:** extracted paths can be registered in a future index while `.fz` blobs
...
if (!$isCli) {
	...
	if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
		...
		try {
			...
			$maxExtract = fzc_web_max_extract_archive_bytes();
			fzc_web_reject_body_too_large($maxExtract);
			fzc_web_require_optional_post_guard();
			if (!isset($_FILES['archive'])) {
				fzc_web_send_json(array('ok' => false, 'error' => 'No archive field: send multipart field `archive` with the .fz file.'), 400);
			}
			$reported = (int) ($_FILES['archive']['size'] ?? 0);
			if ($maxExtract > 0 && $reported > $maxExtract) {
				fzc_web_send_json(array(
					'ok' => false,
					'error' => 'Archive exceeds the ' . (string) $maxExtract . '-byte extract web cap (see FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES; default 10× FZC_WEB_MAX_UPLOAD_BYTES). Use the CLI or raise PHP/nginx upload limits.',
				), 413);
			}
			$origName = (string) ($_FILES['archive']['name'] ?? '');
			$leaf = $origName !== '' ? basename(str_replace('\\', '/', $origName)) : 'archive.fz';
			$leafLow = strtolower($leaf);
			// accept .fz (current) and legacy .fzc uploads
			if (!(strlen($leaf) >= 4 && str_ends_with($leafLow, '.fz'))
				&& !(strlen($leaf) >= 5 && str_ends_with($leafLow, '.fzc'))) {
				fzc_web_send_json(array('ok' => false, 'error' => 'File must have a .fz extension.'), 400);
			}
			...
			if ($maxExtract > 0) {
				...
				if (is_int($onDisk) && $onDisk > $maxExtract) {
...
if ($argvList === [] || $argvList[0] === '-h' || $argvList[0] === '--help') {
	echo "Usage: php fzc_extract.php <file.fz> [output_directory]\n";
	exit($argvList === [] ? 1 : 0);
}
...
function fzc_extract_render_web_ui(string $handlerBase, string $handlerPath): void {
	...
	$maxBytesJson = bench_json_encode_try(fzc_web_max_extract_archive_bytes(), false, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS);
...
<body>
<h1>Extract <code>.fz</code></h1>
<p class="intro">Drop a <code>.fz</code> archive <strong>anywhere on this page</strong>, or click the zone below. Progress appears only while uploading and while the server is extracting. Each line shows the member’s <strong>path as stored in the archive</strong> (whatever was used when it was compressed, e.g. <code>includes/sidebar.php</code> or a single-level name like <code>sidebar.php</code>), not the server job folder. <strong>Web demo:</strong> single-archive size is capped separately from compress (see footer); larger files need the CLI or higher caps.</p>
<p class="warn-long" id="sizeWarn" hidden role="status"></p>
<div class="dropzone" id="dz" tabindex="0" role="button" aria-label="Drop .fz here">
	<p><strong>Drop <code>.fz</code> here</strong> (or anywhere on the page)</p>
	<p>or click to select</p>
</div>
<input type="file" id="pick" accept=".fz,application/octet-stream" class="visually-hidden" aria-hidden="true" />
...
<div id="result"></div>
<footer>
	<p><strong>CLI unchanged:</strong> <code>php {$h} /path/to/archive.fz</code></p>
	<p><strong>Limits:</strong> PHP must allow the upload (copy <code>examples/.user.ini</code> beside these scripts, use <code>examples/.htaccess</code> on Apache, or raise <code>upload_max_filesize</code> / <code>post_max_size</code> in <code>php.ini</code>; built-in server example: <code>php -d upload_max_filesize=80M -d post_max_size=96M …</code>). This page caps one <code>.fz</code> at <code>FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES</code> when set, otherwise <strong>10×</strong> <code>FZC_WEB_MAX_UPLOAD_BYTES</code> (defaults: 80&nbsp;MiB extract vs 8&nbsp;MiB compress tree cap; set either to <code>0</code> to disable that cap). Jobs use <code>web_jobs/</code> next to these scripts if writable, else a temp subfolder; set <code>FRACTAL_ZIP_WEB_JOBS</code> to override.</p>
	<p><strong>Local vs live:</strong> archives built on your machine may use <code>zpaq</code>, <code>7z</code>, or <code>arc</code> outers — the server needs the same tools. Operators: <a href="fzc_capability_report.php">host capability report</a> · repo <code>docs/WEB_LOCAL_PARITY.md</code>.</p>
</footer>
<script>
(function () {
	...
	const FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES = {$maxBytesJson};
	...
	function extractFileOverDemoLimit(f) {
		return !!(f && FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES > 0 && (f.size || 0) > FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES);
	}
	...
	function ingestExtractDrop(dt) {
		...
		if (found) {
			if (extractFileOverDemoLimit(found)) {
				...
				status.textContent = 'Archive exceeds the ' + FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES + '-byte extract cap. Use the CLI for larger archives. On your own server, set FZC_WEB_MAX_EXTRACT_ARCHIVE_BYTES or FZC_WEB_MAX_UPLOAD_BYTES (extract defaults to 10× upload cap; 0 disables) and raise PHP/nginx upload limits — see footer.';
			...
			file = null;
			status.classList.add('error');
			status.textContent = 'Need a .fz file (none found in drop).';
			refreshExtractHeavyWarning();
		}
	}
	...
	pick.addEventListener('change', () => {
		...
		} else { file = null; go.disabled = true; status.classList.toggle('error', !!f); status.textContent = f ? 'File must end in .fz' : ''; }
		refreshExtractHeavyWarning();
	});
	...
	go.addEventListener('click', () => {
		...
		xhr.onload = () => {
			...
			if (!j.ok) { status.classList.add('error'); status.textContent = formatServerError(j); return; }
			...
			if (archiveMemberPathsLookFlat(j.files, j.paths_from_container)) {
				...
				hint.textContent = 'This archive stores every member at one level (file names only—no includes/ or styles/ prefixes). That is what is inside the .fz; the extractor is not hiding folders. To keep a tree layout, compress the parent directory of your project again (web: folder picker on the site root; CLI: php fzc_compress.php /path/to/site).';
